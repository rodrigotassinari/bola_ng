...
xml.rss('version' => '2.0',
  ...
  xml.channel do |xml|
    xml.title("#{Settings.site.name} :: Lifestream :: Tag: #{@tag.name}")
    xml.description("#{Settings.site.tagline}")
    xml.link(tag_name_url(@tag.name))
    ...
    cache ['posts_by_tag_rss', @tag.name, @posts.total_entries, @posts.current_page].join('|').gsub(' ', '-') do
      @posts.each do |post|
        xml.item do
