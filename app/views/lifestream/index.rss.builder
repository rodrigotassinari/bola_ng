...
xml.rss('version' => '2.0',
  ...
  xml.channel do |xml|
    xml.title("#{Settings.site.name} :: Lifestream")
    ...
    cache ['lifestream_index_rss', @posts.total_entries, @posts.current_page, @posts.first.id, @posts.first.updated_at.to_s(:db)].join('|').gsub(' ', '-') do
      @posts.each do |post|
        xml.item do
