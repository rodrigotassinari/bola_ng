...
xml.rss('version' => '2.0',
  ...
  xml.channel do |xml|
    xml.title("#{Settings.site.name} :: Lifestream :: #{@service.name}")
    xml.description("#{Settings.site.tagline}")
    xml.link(lifestream_url(@service))
    ...
    cache ['lifestream_show_rss', @service.name, @posts.total_entries, @posts.current_page, @posts.first.id, @posts.first.updated_at.to_s(:db)].join('|').gsub(' ', '-') do
      @posts.each do |post|
        xml.item do
          xml.title("Eu #{t post.service_action} no #{@service.name}: #{h(post.title)}")
          xml.description do
